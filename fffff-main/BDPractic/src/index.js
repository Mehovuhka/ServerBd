...
App.get("/get/readers", async (req, res) => {
  console.log("get");
  const get = await connection.connection.query(`
    SELECT fio, adres, passport, phone FROM readers
    `);
  console.log(get);
...
App.post("/post/readers", async (req, res) => {
  console.log("post readers");
  const readers = req.body;
  const post = await connection.connection.query(`
        INSERT INTO READERS(fio, adres, passport, phone) VALUES
        (${JSON.stringify(readers.fio)}, ${JSON.stringify(
    readers.adres
  )}, ${JSON.stringify(readers.passport)}, ${JSON.stringify(readers.phone)})
    `);
  console.log(post);
  res.setHeader("Access-Control-Allow-Origin", "*");
  res.setHeader("Access-Control-Allow-Headers", "origin, content-type, accept");
